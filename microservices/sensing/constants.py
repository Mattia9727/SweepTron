...
def update_all():
    global print_debug, iq_mode, grafici_dir, logs_dir, measures_dir, iq_measures_dir, settings_path, sensing_activity,\
        ...
        time_search_for_adjust_ref_level_scale, iq_length_value, iq_length_unit, iq_bits, force_if_gain, delay_time, num_of_averages, resolution_bw, video_bw
    ...
    # Estrai le costanti
    num_of_averages=constants["num_of_averages"] #numero di medie
    resolution_bw=constants["resolution_bw"] #banda di risoluzione IN HZ
    video_bw=constants["video_bw"] #banda video IN HZ
    delay_time=constants["delay_time"]  #tempo di delay minimo per fare lo scanning di tutto il numero di medie, in secondi
