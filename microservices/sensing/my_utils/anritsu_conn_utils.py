...
# TCP_IP = '192.168.0.2'
TCP_IP = '10.0.0.2' #PER ANALIZZATORE ARPA
...
def setup_anritsu_device_rack(conn):
    send_command(conn,':MODE SPEC\n')#imposta il tipo di modalità spectrum analyzer
    send_command(conn,':CONFigure:CHPower:CONFigure:CHPower\n')#configura il CHpower
    send_command(conn,':FSTRength:STATe 1\n')
    send_command(conn,':FSTRength:ANTenna "{}"\n'.format(c.antenna_file))#no
...
def setup_anritsu_device_MS27102A(conn):
...
def setup_for_single_freq(conn,f):
    ...
    send_freq_start = 'sens:freq:star {}\n'.format(selected_frequency_start)
    send_command(conn, send_freq_start)
    selected_frequency_stop = c.frequency_stop[f]
    send_freq_stop = 'sens:freq:stop {}\n'.format(selected_frequency_stop)
    send_command(conn, send_freq_stop)



    res_bw = 'sens:band:res {}\n'.format(c.resolution_bw)
    send_command(conn, res_bw)  

    vid_bw = 'sens:band:vid {}\n'.format(c.video_bw)
    send_command(conn, vid_bw)

    #frequenc_range_str = ':SENS:FREQ:STAR {} MHZ;:SENS:FREQ:STOP {} MHZ\n'.format(selected_frequency_start,selected_frequency_stop)
    #send_command(conn, frequenc_range_str)  # Frequency rang
